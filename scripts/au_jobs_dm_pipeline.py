...
def preflight_prospeo(api_key: str) -> None:
    """Validate Prospeo X-KEY + credits before Apify so a bad key/empty balance does not burn scrape credits."""
    try:
        resp = requests.get(
            f"{PROSPEO_API}/account-information",
            headers={"X-KEY": api_key, "Content-Type": "application/json"},
            timeout=30,
        )
    except requests.RequestException as exc:
        raise PipelineError(f"Prospeo preflight network error: {exc}") from exc

    if resp.status_code == 429:
        raise PipelineError(f"Prospeo rate limit (HTTP 429): {resp.text[:500]}")
    if resp.status_code >= 400:
        raise PipelineError(
            f"Prospeo preflight failed (HTTP {resp.status_code}): {resp.text[:500]}"
        )

    try:
        body = resp.json()
    except Exception as exc:
        raise PipelineError(f"Prospeo preflight invalid JSON: {exc}") from exc

    info = body.get("response") if isinstance(body, dict) else None
    if not isinstance(info, dict):
        info = body if isinstance(body, dict) else {}

    remaining = info.get("remaining_credits")
    plan = info.get("current_plan")
    renews = info.get("next_quota_renewal_date")
    print(
        f"[Prospeo] preflight OK plan={plan} remaining_credits={remaining} renews={renews}"
    )

    # Account credits can be 0 even when daily search rate headers still show remaining.
    if remaining is not None and int(remaining) <= 0:
        raise PipelineError(
            "Prospeo insufficient credits: remaining_credits="
            f"{remaining} plan={plan} renews={renews}. "
            "Top up or wait for renewal before re-running (Apify scrape skipped)."
        )


def preflight_instantly(api_key: str) -> None:
    """Detect Instantly lead-upload limit before Apify/Prospeo spend.

    Instantly returns HTTP 403 with 'Lead limit reached' when remaining uploads
    are 0. Probe with a disposable example.com address so we stop early instead
    of burning scrape/enrichment credits.
    """
    probe_email = "au-pipeline-preflight-probe@example.com"
    try:
        resp = requests.post(
            ...
            headers={
                ...
                "User-Agent": "au-jobs-dm-pipeline/1.0",
            },
            json={
                ...
                "leads": [
                    {
                        ...
                        "last_name": "Probe",
                        "company_name": "PipelinePreflight",
                    }
                ],
        ...
        raise PipelineError(f"Instantly preflight network error: {exc}") from exc

    text = (resp.text or "")[:800]
    lower = text.lower()
    if resp.status_code == 429:
        raise PipelineError(f"Instantly rate limit (HTTP 429): {text}")
    if resp.status_code == 403 and (
        "lead limit" in lower or "reamining uploads" in lower or "remaining uploads" in lower
    ):
        raise PipelineError(
            "Instantly lead limit reached (remaining uploads: 0). "
            "Raise Instantly plan/lead-limit before re-running "
            "(Apify + Prospeo skipped to avoid burning credits). "
            f"Detail: {text}"
        )
    if resp.status_code >= 400:
        # Other Instantly errors (auth, campaign missing, etc.) also block the run.
        raise PipelineError(
            f"Instantly preflight failed HTTP {resp.status_code}: {text}"
        )

    data = resp.json() if resp.content else {}
    uploaded = data.get("leads_uploaded", 0)
    skipped = data.get("skipped_count", 0)
    duplicated = data.get("duplicated_leads", 0)
    print(
        "[Instantly] preflight OK "
        f"(probe uploaded={uploaded} duplicates={duplicated} skipped={skipped}; "
        "campaign will NOT be activated)"
    )
...
def main() -> int:
    ...
    try:
        ...
        preflight_prospeo(secrets["PROSPEO_API_KEY"])
        preflight_instantly(secrets["INSTANTLY_API_KEY"])
