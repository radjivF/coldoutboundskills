...
def preflight_prospeo(api_key: str) -> None:
    """Validate Prospeo X-KEY before Apify so a bad key does not burn scrape credits."""
    try:
        resp = requests.get(
    ...
    if resp.status_code >= 400 or body.get("error"):
        code = body.get("error_code") or f"HTTP_{resp.status_code}"
        raise PipelineError(
            f"Prospeo preflight failed ({code}): {resp.text[:500]}. "
            "Rotate PROSPEO_API_KEY in Automation Secrets (X-KEY from Prospeo dashboard)."
        )

    print("[Prospeo] preflight OK (account-information)")
